# ZPK Digital ID Gemma 4 Good Demo Dataset
...
The demo uses synthetic data only. It shows local ZPK identity registration, DID-style and Android Keystore-backed HMAC-SHA256 recovery artifacts, a signed agent ledger, selective disclosure claims, CUI risk lookup, and Spanish recovery guidance. It includes `.env.example` but not `.env`, and it does not embed a Gemini API key.
